###############################################################################
# 02_spread_analysis.R — Bid-Ask Spread Dynamics at Microsecond Resolution
...
# overweighting periods of rapid quote updates.
# ═══════════════════════════════════════════════════════════════════════════════
...
# Group by time-of-day to detect the well-known U-shape:
#   - Wide spreads at open (high uncertainty)
#   - Narrow spreads mid-day (steady state)
#   - Widening at close (inventory risk)
# ═══════════════════════════════════════════════════════════════════════════════
...
# Compare median spreads across symbols and test for significant differences.
# ═══════════════════════════════════════════════════════════════════════════════
...
cross_sectional_analysis <- function(dt_book) {
  ...
  # Kruskal-Wallis test: non-parametric test for spread differences across symbols
  # H0: All symbols have the same spread distribution
  # Use Kruskal-Wallis because spread distributions are typically right-skewed
  ...
  if (kw_test$p.value < 0.05) {
    log_msg("Significant differences in spreads across symbols (p < 0.05)")
  } else {
    log_msg("No significant differences in spreads across symbols")
  }
...
generate_spread_plots <- function(dt_book, intraday, twas_dt) {
  ...
  p2 <- ggplot(spread_dist, aes(x = reorder(symbol, spread_bps, FUN = median),
    ...
    labs(
      ...
      x = "Symbol",
      y = "Spread (basis points)",
      caption = "Kruskal-Wallis test for distributional differences"
    ) +
    THEME_DARK +
...
main_spread_analysis <- function() {
  ...
  # Cross-sectional analysis with statistical tests
  cross <- cross_sectional_analysis(result$book)
  log_msg("Cross-sectional summary:")
  print(cross$summary)
  ...
  if (!is.null(result$trades)) {
    trade_metrics <- result$trades[, .SD, .SDcols = intersect(
      names(result$trades),
      c("timestamp", "symbol", "trade_price", "mid_price",
